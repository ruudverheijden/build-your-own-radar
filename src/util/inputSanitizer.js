...
const InputSanitizer = function () {
  ...
  self.sanitize = function (rawBlip) {
    ...
    blip.iconUrl = sanitizeHtml(blip.iconUrl, restrictedOptions)
  ...
  self.sanitizeForProtectedSheet = function (rawBlip, header) {
    ...
    const iconUrlIndex = header.indexOf('iconUrl')
    ...
    const iconUrl = iconUrlIndex === -1 ? '' : blip[iconUrlIndex]
    ...
    blip.iconUrl = sanitizeHtml(iconUrl, restrictedOptions)
